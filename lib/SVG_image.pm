...
sub create_image_SVG {
    ...
    if (defined $options{strain_reorder_file} || defined $options{phylogenetic_file}) {
        ...
        foreach my $part_reorder (sort {$a<=>$b} keys %strain_reorder_hash){ 
            ...
            foreach my $tbl_part_file(sort @dir_part_TFT){ 
                ...
                if(($tbl_part_file =~ /$check_value/) and ($tbl_part_file =~ /part$/)) { # delete temp file generated when part_TFT file were edited to modify the genename
    ...
    open (SVG_MAP, ">$workplace/figure.svg") or die("Cannot open file for writing");
